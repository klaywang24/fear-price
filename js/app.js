...
<!-- 2026-07-26 用户裁（§41）：两段压成一行 —— 品牌句留一句小字；「官方定义」段挪去 K 指数页
     ...
     「GitHub 时间戳」半句删=与下方验证卡文案重复。 -->
        ...
        <div class="ledger-cards">
          <a class="ledger-card" href="kindex">
            ...
            <div class="lc-state ${kTrig ? "neg" : "pos"}">${kTrig ? "触发中" : "未触发"} <span>（K &lt; 1 触发）</span></div>${kd.current.provisional ? '<div class="lc-prov">当晚读数，次日按 CNN 定稿更新</div>' : ""}
    ...
    const CRISES = [
      ...
      ["2022-09", "2022", "加息", "below", -8, 22],   // 09-23（dx 0 时 375px 宽右缘仍多 1px，改 −8 让标签右缘正好落在点上）：本点 x≈96% 走 edge-r（标签右缘贴点），dx 再往右推 44px 就被 .pulse-hero overflow:hidden 切掉（1000px 宽实测切 20px）；edge-r 的点 dx 只能 ≤0
    ];
